...
- Il blocco centrale sul candidato: scegli UN SOLO risultato misurabile dal CV — il più rilevante per QUESTO annuncio specifico — e usalo come prova a supporto di un'affermazione, non come elenco. Vietato enumerare più di un risultato quantitativo nello stesso paragrafo. Limite di MASSIMO 2 valori quantificati in tutta la lettera — ma questo limite si applica SOLO a importi in euro (€, budget, RAL, risparmi) e percentuali (%) che rappresentano un risultato o traguardo ottenuto, sia scritti in cifre ("22%", "€2,5M") sia scritti in lettere ("ventidue percento"). Anni di esperienza, dimensione di team gestiti, numero di progetti, o altri dati anagrafici/descrittivi sul candidato o sul contesto NON contano ai fini di questo limite (es. "otto anni", "eighteen people" sono sempre liberamente citabili) e non vanno mai confusi con un risultato quantificato.
  ...
  if (sub) {
    ...
    if (!reserved.ok) {
      ...
      const message = sub.tier === "trial"
        ? `Hai raggiunto il limite di ${limits?.cover_letters_per_month ?? "?"} lettere di motivazione incluse nel trial. Sottoscrivi un piano per continuare.`
        : `Hai raggiunto il limite di ${limits?.cover_letters_per_month ?? "?"} lettere di motivazione mensili per il piano ${sub.tier}. Aggiorna il piano per continuare.`;
      return NextResponse.json({
        error: message,
        code: "limit_reached",
